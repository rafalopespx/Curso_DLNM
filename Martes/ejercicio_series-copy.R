...
# cargar funciones R
source("../rafa/Dropbox/UPCXIVSC_RST/2 Martes/Datos/QAICM.R")
source("../rafa/Dropbox/UPCXIVSC_RST/2 Martes/Datos/findmin.R")
...
data_valencia <- dir_ls("../rafa/Dropbox/UPCXIVSC_RST/2 Martes/Datos", regexp = "csv$") %>% 
                    map(read.csv2) %>% 
...
# duplicados
is_duplicated(data_valencia, index = fecha) # FALSE

# lagunas
has_gaps(data_valencia) # FALSE
...
data_valencia <- mutate(data_valencia, 
                        ...
                        wk = if_else(wd %in% c("Sunday", "Saturday"), 1, 0),
                        trend = row_number())

# exportar *.csv
write_csv2(data_valencia, "~/Desktop/Curso_DLNM/Martes/data_valencia_clean.csv")

# guardar *.RData
save(data_valencia, file = "~/Desktop/Curso_DLNM/Martes/data_valencia_clean.RData")
...
pred1 <- crosspred(btmean1, model.tmean1, cen=min1) # MMT onde está? function that find it
...
model.tmean4 <- glm(tm ~ spl + btmean4 + as.factor(dow), data=data_valencia, family=quasipoisson)
