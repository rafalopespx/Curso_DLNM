...
# 0) pasos previos
     ...
     # 0.1) traslado los datos a escala en percentiles.
      ...
      for(i in 1:len){
            Fd<-ecdf(dCV[[i]]$tmean) # esta ordenando la cumulativa distribucción
            dCV[[i]]$ptmean<-Fd(dCV[[i]]$tmean)} # temperatura en la escala de percentiles
      ...
      tpred<-(1:99)/100 # haciendo tpred a priori, por se estar en la escala relativa
...
plot_quantile
 # Cstl y vlnc tienes el punto de MMT mas elevado, y Alcn tiene el punto de MMT mas bajo      
      
gg<-ggpubr::ggarrange(plot_raw, plot_quantile)
