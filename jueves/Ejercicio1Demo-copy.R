...
# Parametros para cada dlnm de cada cidade
varper<-c(10,50,90) # lugar de cada knots
varfun<-"bs" # tipo de spline para variavel

nlag<-21 # nº de lags
xlag<-0:nlag # tamanho de lags
lagnk <- 3 # nº de knots nos splines de lags
klag<-logknots(nlag,lagnk) # colocação de knots nos lags 
lagfun<-"ns" # spline para os lags
...
# Bucle para el análisis
  ...
  for(i in 1:len) {
      ...
      #predicción reducida: overall
       red[[i]] <- crossreduce(cb,model,at=tpred) # quiereis lo efecto overall
       coef[i,] <- red[[i]]$coef
       vcov[[i]] <- red[[i]]$vcov
