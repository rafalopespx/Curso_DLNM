...
# Crea caracteres de forma automática, evaluado las variables que se quedan em {}
...
subset_europe <- filter(residential_mobility, 
                        is.na(sub_region_1),
                        !is.na(resi)) %>% # Filtrar datos para países pero no para sub-regiones
  left_join(wld, by = c("country_region_code"="iso_a2")) %>% # combinando colunas con nombres diferentes
  filter(subregion %in% c("Northern Europe",
                          "Southern Europe",
                          "Western Europe",
                          "Eastern Europe")) %>% # Filtrando las sub-reigones de Europa
...
## Un subset para selecionar datos con mobilidade

# tabla ancha
mobi_wide <- pivot_wider(mobility_selection, 
                         names_from = country_region_code,
                         values_from = resi)
# Cambiar de longer format para format wider
mobi_wide
...
) # Extraendo las causas y los outcome de la misma coluna
...
map_df(vec_list, mean)
map_chr(vec_list, mean)
map_dfc(vec_list, mean)
walk(vec_list, mean)
