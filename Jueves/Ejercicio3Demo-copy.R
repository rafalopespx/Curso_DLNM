...
load("Dropbox/UPCXIVSC_RST/4 Jueves/Datos/datosSC.RDATA")
dCV<-dCV_94_06
...
# 0) pasos previos (igual que 2!!)
     ...
     # 0.1) traslado los datos a escala en percentiles.
      ...
      bvar <- do.call("onebasis",c(list(x=tpred),attr(cb,"argvar"))) # es la basis completa
...
#Etapa 1) Analizamos en las tres ciudades, guardando coeficientes y sus matrices de var-cov (map o bucle)
  ...
  mmp<-0.95 # para efecto del calor y cambiar value = 0.95
  # mmp<-0.05 # para efecto del frio y cambiar value = 0.05
  
  # Piensar en esto, se hace sintido, para el calor nos centramos en el 95th percentile, entonces tenemos que pegar todo los percentiles depués de 95th.
  # para el frio nos centramos en el 5th percentile, entonces tenemos que pegar todo los percentiles del antes de 5th.
  ...
  for(i in 1:len) {
      ...
      # Voy a mirar en todo los lags, se voy hacer esto, necesito mirar en un vlaor de temperatura de exposición
      # Voy a dibujar en el Percentil 95
