...
data <- read.dta("Dropbox/UPCXIVSC_RST/5 Viernes/Datos/misdatos_valencia.dta")
str(data)
# tm0 es numero de fallecidos con menos de 65 años, y tm1 es con más
...
cbind(model.cco1$coefficients[2], sqrt(summary(model.cco1)$cov.scaled[2,2]), time1)
cbind(model.cco2$coefficients[2], sqrt(summary(model.cco2)$cov.scaled[2,2]), time2)
cbind(model.cco3$coefficients[1], sqrt(summary(model.cco3)$cov.scaled[1,1]), time3)
...
summary(model.age3)
(exp(model.age3$coefficients[1]*10)-1)*100 # efecto en los jóvenes
(exp((model.age3$coefficients[1]+model.age3$coefficients[4])*10)-1)*100 # el efecto en los jóvenes añadindo el efecto para los mayores
...
# define heatwave based as the 95th percentile of the mean temperature distribution
